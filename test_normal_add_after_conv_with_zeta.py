...
# This code adds Normal Distribution Noise after the Convolution Outputs with Zeta as a Scaling Factor
...
def main():
   # Set Parser
    ...
    parser.add_argument('--noise', default=1.0, type=float, help='noise')
    ...
    # net_path = './normal_models/nest_cifar_net_epoch_200' + '.pth'
    ...
    net.eval()
     ...
     # Defect Model (Add Normal Distribution Noise), Alpha denotes different std
    def hook(model, input, output):
        mean = 0.0
        alpha = args.noise
        defect = torch.normal(mean, alpha, size=output.size()).to(device)
        zeta = torch.mean(output, dim=(0,1,2,3))
        output = output + defect * zeta
        return output
